...
| 105 | SEC-011 | Credenziali amministrative in chiaro dentro `Helpdesk_ABBYY/ABBYY.docx` su OneDrive: password Administrator del license server (ripetuta tre volte), password della VM Windows citata come VM101, credenziali complete dell'utente guest della condivisione `abbyy_15` (utente creato senza scadenza password e noto a tutti i dipendenti per le re-installazioni). Stessa radice di SEC-007/SEC-010: segreti operativi nel filesystem condiviso invece che in un vault. Le credenziali non sono mai riportate nei file tracciati | ABBYY.docx (03/07/2026) → 2025-q1-server-vianova.md §Migrazione licenze ABBYY |
| 106 | SRV-001 | Incoerenze da verificare nel sorgente ABBYY: il nuovo server e' citato con due hostname diversi (refuso probabile in una delle due forme; hostname reali nella mappa privata) e compare una "VM101 (node pve, Windows)" con password propria che non esiste nell'inventario snapshot v3 (il Windows Server 2022 e' VM100). Verificare se VM101 e' stata una VM intermedia poi rimossa o un refuso per VM100 | ABBYY.docx (03/07/2026) vs snapshot v3 |
| 107 | SRV-002 | Host IntraLino: la documentazione benchmark descrive 10.61.20.58 (Ollama) e 10.61.20.60 (stack Docker) come "due VM Proxmox", ma attribuisce alla .58 hardware fisico dedicato (i7-7700, scheda madre, PSU, GPU RTX 5060 Ti installata fisicamente l'08/06/2026): con ogni probabilita' la .58 e' una macchina fisica separata, non una VM. Nessuno dei due host compare nell'inventario snapshot v3 (VM602 ITdeveloping e' su vmbr3). Chiarire natura e collocazione dei due host e aggiornare l'inventario | CLAUDE_STATO_PROGETTO.md (benchmark IntraLino) vs snapshot v3 |
