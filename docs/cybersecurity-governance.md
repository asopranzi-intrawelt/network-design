...
| Data | Evento | Dettaglio |
|------|--------|-----------|
| 03/07/2026 | BitLocker attivo sugli endpoint (copertura non totale, vedi nota) | Cifratura full-disk XTS-AES 128 bit sui dischi principali delle macchine aziendali Windows 11 **Correzione del 07/09/2026**: la copertura non era totale. L'attivazione dell'automazione di cifratura su due postazioni Windows 11 Pro quel giorno ha riportato "cifratura avviata", e il pannello del dispositivo dichiarava lo stato BitLocker disabilitato: quei due dischi erano quindi **non cifrati**, non semplicemente privi di deposito della chiave. L'affermazione "tutti gli endpoint" era falsa almeno per loro. Il perimetro va dichiarato per misura e non per intenzione, e la misura va rifatta a cifratura completata, perche' fino ad allora lo stato di protezione resta Off (SEC-044, #188).; chiavi di ripristino in escrow centralizzato su NinjaOne, accesso riservato all'IT previa autenticazione per solo disaster recovery. |
...
Sul fronte endpoint la copertura at-rest e' invece cambiata il 03/07/2026 con l'attivazione di BitLocker su tutti i dischi principali delle macchine aziendali **Correzione del 07/09/2026**: la copertura non era totale. L'attivazione dell'automazione di cifratura su due postazioni Windows 11 Pro quel giorno ha riportato "cifratura avviata", e il pannello del dispositivo dichiarava lo stato BitLocker disabilitato: quei due dischi erano quindi **non cifrati**, non semplicemente privi di deposito della chiave. L'affermazione "tutti gli endpoint" era falsa almeno per loro. Il perimetro va dichiarato per misura e non per intenzione, e la misura va rifatta a cifratura completata, perche' fino ad allora lo stato di protezione resta Off (SEC-044, #188). (XTS-AES 128 bit, chiavi di ripristino in escrow centralizzato su NinjaOne, non possedute dall'utente): il problema descritto in questa sezione resta quindi circoscritto agli archivi storici cifrati e alla custodia delle loro password, non alla cifratura delle postazioni.
