...
> **Nota del 06/08/2026.** Questa scheda descrive la topologia logica. Il livello fisico ed elettrico corrispondente — quale apparato sta in quale armadio, quale porta dello switch porta quale apparato, da dove ciascuno prende corrente e che cosa cade se cade un gruppo di continuita' — vive ora in `docs/livello-fisico-ed-elettrico.md`, aperta con il documento `STATO RETE INTRAWELT.docx` dell'IT Manager. Le due schede vanno lette insieme quando si tocca la catena WAN o gli armadi. L'IT Manager ha inoltre dichiarato **superato** il diagramma di livello 2 presente in repository. Dal 06/08/2026 esiste la **mappa interattiva** `docs/network-map.html`, generata da `data/network-topology.json` secondo ADR-023: e' l'artefatto che si aggiorna a ogni intervento invece di invecchiare, e va corretta modificando la fonte e rigenerando, mai a mano. Il diagramma ASCII di questa scheda resta come lettura rapida a colpo d'occhio, ma quando le due divergono vale la mappa, che ha una fonte verificabile.
