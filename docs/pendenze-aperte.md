...
| La cache del controller RAID e' spenta **perche' il modulo che la protegge e' guasto**, o per un'altra ragione? Il registro del kernel dichiara la cache disabilitata, che e' misurato; la causa e' dedotta. E' la voce che giustifica o non giustifica una finestra di spegnimento | recuperare `ssacli` e leggere `ctrl all show status`, con lo stato di batteria o condensatore. Nello stesso passaggio: dischi meccanici o a stato solido, e driver `hpsa` contro `smartpqi` | STOR-005 (#200); asserzione `PVE-CACHE-RAID` in `data/scadenze.json` |
