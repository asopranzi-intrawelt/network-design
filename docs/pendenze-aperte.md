...
> Vista consolidata di cio' che resta da sanare, aggiornata al 16/09/2026. Non e' una fonte: ogni riga punta al registro che la possiede davvero, e nessuna informazione vive **solo** qui. Serve a rispondere in trenta secondi alla domanda "cosa e' rimasto in sospeso", che altrimenti richiede di attraversare quattro registri diversi: i gap in `infrastructure-timeline/GAP-TBC.md`, i micro-interventi in `interventi-robustezza.md`, i micro-step in `.claude/context/roadmap.md` e le decisioni in `.claude/memory/decisions.md`. Va riletta e potata a fine sessione: una lista di pendenze che non si accorcia mai smette di essere letta.
...
|---|---|---|
| Licenziare con Nebula Professional il nuovo access point **entro il 24/09/2026** (adottato il 09/09/2026), co-terminato al 22/11/2027 | M13c-9; ADR-022; precedente NEB-002 (#140); scadenza datata in `data/scadenze.json` | Il livello dell'organizzazione e' il minimo fra i suoi apparati: un dispositivo scoperto fa cadere tutta l'organizzazione a Base Pack e spegne la OpenAPI. E' gia' successo il 05/08. **Da pendenza generica e' diventata una data**, perche' l'adozione e' avvenuta e i quindici giorni corrono |
| ~~**Non adottare il WBE530 in Nebula finche' la restituzione e' aperta**~~ — **superata il 09/09/2026 dalla decisione dell'IT Manager**: l'apparato si tiene, non si restituisce, e va in servizio con una protezione da esterno | M13c-6; ADR-022 | La cautela era giusta finche' il reso era aperto: adottare fa partire i quindici giorni di grazia su un apparato che potrebbe tornare indietro. Chiusa la questione del reso, la cautela non ha piu' oggetto e la conseguenza dell'adozione e' solo la scadenza della riga sopra |
| ~~**Rifare la scelta dell'hardware della tratta esterna**~~ — **chiusa il 09/09/2026 per decisione**: si tiene il WBE530 dichiarato `Indoor Use only` e gli si mette una protezione da esterno; il rischio residuo e' tracciato come #197 | M13c-3, M13c-6; verifiche del 07/08/2026; ADR-029 | Il requisito "da esterno con due porte" **non era soddisfacibile** nella linea Nebula, e l'architettura che regge e' proprio quella scelta con ADR-029, cioe' access point piu' **switch PoE gestito** al posto del GS-105B. Cio' che resta e' un apparato per uso interno installato all'esterno, che non e' un errore di architettura ma un rischio ambientale da presidiare |
...
| **Finestra di manutenzione per il modulo di cache del controller RAID del nodo Proxmox.** Richiede lo spegnimento del nodo e quindi di tutte le macchine virtuali: non e' sostituibile a caldo. Non si pianifica prima della conferma diagnostica qui sotto | aperta dal 16/09/2026 | STOR-005 (#200); ELE-003 (#147) per il vincolo del singolo alimentatore |
...
| **Il crollo di lettura non viene ne' dai dischi ne' dal controller: da dove viene?** Misurato il 16/09/2026 che batteria, controller e tre SSD sono tutti sani. Venti MiB/s su tre SSD in RAID 5 non e' un rallentamento ma un valore due ordini di grandezza sotto la portata dell'insieme, quindi il collo di bottiglia sta sopra il ferro, fra `vzdump` e il volume sottile | la lettura grezza a offset crescenti, `dd iflag=direct` su `/dev/pve/vm-100-disk-1` da 0 a 160 GB: piatta e veloce accusa `vzdump` e la catena thin, piatta e lenta accusa lo strato LVM | STOR-005 (#200); asserzione `PVE-LETTURA-LENTA` |
| **Il server ha davvero otto anni e mezzo, e con quale garanzia?** La scheda madre e' datata 20/02/2018 e il registro eventi porta guasti del proprietario precedente con orologio verificato corretto: del 2025 e' la fornitura, non l'hardware | numero di serie dello chassis, che vive in `_notes/`, nel portale di supporto del costruttore; e rilettura della fornitura del 2025 per vedere se dichiarasse hardware usato o ricondizionato | SRV-009 (#201); M28-5; asserzione `SRV-ETA-SERVER` |
| **I due alimentatori del server stanno su due linee distinte?** I sensori dicono che gli alimentatori sono due e sani, contro ELE-003 (#147) che ne dichiara uno; ma due alimentatori sulla stessa presa danno ridondanza di apparato e non di alimentazione, e la differenza e' tutta la' | sopralluogo al retro dell'armadio, incrociando con i gruppi di continuita' di ELE-001 | ELE-005 (#202); ELE-001 (#145); M28-5; asserzione `SRV-ALIMENTATORI` |
| Gli otto lavori di backup notturni completano tutti da quando l'attesa sul lock e' a ventiquattro ore, e a che ora finisce l'ultimo? | pagina dei task del nodo, non attesa di una mail: la notifica scatta solo sul fallimento, e il fallimento e' stato reso quasi impossibile dal rimedio | STOR-005 (#200); asserzione `PVE-BACKUP-COMPLETI` |
| Quanto dura davvero il ripristino di una macchina virtuale dal deposito su NAS INTRA? | ripristinare una macchina piccola e spenta cronometrando, e rifarlo dopo la riparazione per avere i due numeri | `business-continuity-disaster-recovery.md`; asserzione `PVE-RIPRISTINO-TEMPO` |
...
| Smontare l'immagine SPP lasciata agganciata sul nodo Proxmox durante l'indagine del 16/09 (`umount /mnt/spp`) | Montata in sola lettura per cercare `ssacli`, che non e' stato estratto. Non fa danno e non va dimenticata: e' l'unica traccia che quella sessione ha lasciato sul sistema oltre alle due modifiche di configurazione |
