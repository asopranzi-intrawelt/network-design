...
#### Promozione del 01/09/2026: R12 torna prerequisito, e questa volta lo e' davvero

La correzione di priorita' riportata qui sotto resta valida per le ragioni che porta: rispetto alle scansioni e alle stampanti R12 non era e non e' un prerequisito. **Il contesto e' pero' cambiato in un modo che quella valutazione non poteva prevedere**, e la promozione va motivata per non sembrare un ripensamento.

Sono cambiate due cose. La prima e' l'emissione dei certificati interni: un certificato certifica un **nome**, il navigatore confronta il nome digitato con quello scritto nel certificato, e un nome che nessuno risolve non e' collaudabile ne' utilizzabile. Senza R12 la catena dei certificati decisa con ADR-024 e ADR-025 non parte. La seconda e' la segmentazione: quando i servizi cambieranno segmento cambieranno indirizzo, e con la risoluzione centralizzata quello spostamento e' un record da modificare, mentre senza e' il file degli host di ogni postazione da toccare, ogni volta.

Va notato che l'argomento contrario riportato sotto — cioe' che in azienda la pratica consolidata sia modificare il file degli host e che quella pratica funzioni e sia conosciuta — resta vero e non e' stato smentito. Cio' che e' cambiato e' il numero di volte che quella pratica andrebbe ripetuta: finche' si trattava di un servizio ogni tanto, il guadagno era piccolo; con una segmentazione che sposta piu' servizi e con certificati che ne dipendono, la stessa pratica diventa un moltiplicatore di lavoro manuale. E' lo stesso difetto strutturale che #171 registra per le liste di indirizzi scritte a mano dentro le configurazioni.

Tre avvertenze operative per l'esecuzione, emerse il 01/09/2026 rileggendo la configurazione del firewall. La rete Wi-Fi del personale distribuisce come server dei nomi due resolver pubblici e non il firewall, quindi i suoi dispositivi non risolverebbero i nomi interni e il difetto si manifesterebbe come "dal cavo funziona e dal Wi-Fi no": va corretto contestualmente. La rete ospiti deve invece restare sui resolver pubblici, e non e' una svista ma la scelta giusta, perche' un ospite non ha ragione di risolvere i nomi dei servizi interni e la risoluzione e' il primo passo di qualunque ricognizione. E i record vanno creati prima di emettere i certificati, perche' altrimenti si collauderebbe un servizio avendo cambiato due cose insieme senza poter sapere quale delle due non funziona.
