...
**Corretto il 16/09/2026 su misura di una sessione parallela, e i numeri dello snapshot v4 non valgono piu'.** I lavori di backup sono **otto** e non nove, sempre in modalita' snapshot con compressione zstd, scaglionati fra le 21:00 e le 03:00: sette verso NAS_INTRA e uno della sola VM 100 verso NAS_HERO, che dalle 00:30 e' stato spostato alle **04:30** per togliere una collisione di orario con un altro lavoro. Ritenzione per lavoro: tre copie giornaliere, una settimanale, una mensile. Notifica per posta **solo sul fallimento**. Il fatto architetturale che nessuna di queste righe diceva, ed e' quello che decide il comportamento sotto carico, e' che i lavori sono una **catena seriale**: il lock di vzdump e' per nodo e vale anche fra storage diversi, quindi non si sovrappongono mai e ciascuno eredita il ritardo del precedente. L'attesa massima sul lock, prima al valore predefinito di centottanta minuti, e' stata portata a millequattrocentoquaranta il 16/09/2026 dopo due fallimenti; vedi #200.
...
| A.8.22 Segregation in networks | Parziale | **Correzione del 25/08/2026, misurata**: cio' che questa riga chiamava "segmentazione fisica (bridge separati)" non e' una segmentazione. I quattro bridge dell'host escono su quattro porte dello switch tutte in accesso con PVID 1, nessun bridge e' VLAN-aware e nessuna macchina porta un tag: sono quattro tratte parallele nello stesso dominio di broadcast (#152/NET-022). Uno dei quattro, quello che porta l'indirizzo di gestione dell'hypervisor, esce per giunta su uno switch non gestito (#157/NET-026). Nessun VLAN tagging ne' policy enforcement lato Proxmox (M4/M5 del piano), il cui disegno va rifatto: dettaglio e fondamento in `docs/virtualizzazione-proxmox-e-bridge.md`; a livello LAN, switch di management erroneamente sulla VLAN Guest (NET-001, M12); **la LAN principale e' un'unica rete flat `/19`** (PC .10, server .20, stampanti .30 sono alias IP, non VLAN separate — NET-009, M22, identificato il 15/07/2026). Implicazione concreta segnalata dall'utente il 16/07/2026: le stampanti multifunzione con scan-to-folder possono raggiungere direttamente le cartelle condivise di un PC Windows 11 invece di essere confinate a una destinazione unica e presidiata (es. un NAS), un fronte di sanificazione dei flussi dato distinto dalla sola segmentazione di rete (si veda anche A.8.3 Information access restriction) — da riprendere quando si pianifica M22. **Tentativo 16/07/2026**: la VLAN
...
| A.8.16 Monitoring activities | Non verificato | Logging traffico di rete non documentato; nessuna policy di raccolta/retention log firewall o switch **Aggiornamento del 16/09/2026, dal primo caso misurato di allarme spento invece che risolto.** Il backup delle macchine virtuali notifica **solo sul fallimento**, e quando due lavori sono falliti per attesa scaduta il rimedio ha portato l'attesa a ventiquattro ore: i lavori ora completano, quindi nessuna mail parte piu', mentre la condizione che li rallentava resta identica e per giunta sconfina nell'orario di lavoro. Non e' un difetto della notifica ma del criterio, cioe' notificare un esito binario invece di una soglia: un backup che impiega quattro volte il suo tempo e' un segnale, e oggi nessuno lo raccoglie. Lo stesso criterio manca su tutta la classe: nessun apparato di questa rete ha oggi un allarme su un valore che degrada, solo su un servizio che cade. Dettaglio in #200 |
...
| A.8.8 Management of technical vulnerabilities | Non verificato | Patch management di switch Nebula, firewall e firmware NAS non documentato (Fase 4 step 1); un VA esterno non credenzialato (06/11/2025, Onova) i cui esiti sono in `vulnerability-assessment-nov2025.md`. **Correzione del 07/09/2026**: questa riga diceva "8 anomalie" e il numero non e' sostenibile da nessuna delle fonti. I due report Nessus consegnati contano 1506 e 415 pagine su venticinque host, e la checklist di remediation derivata (15/05/2026) traccia 174 interventi, di cui venti critici e quattordici alti, piu' un blocco trasversale di azioni organizzative. Va inoltre distinto il piano dal fatto: quella checklist e' uno strumento a stato locale, il file distribuito ha `SNAPSHOT = null` e le spunte vivono nel `localStorage` del browser, quindi non costituisce evidenza di rimedio eseguito |
...
| A.8.1 User endpoint devices | Conforme (endpoint) / Parziale (gestione centralizzata) | BitLocker attivo sugli endpoint Windows dal 03/07/2026, chiavi in escrow su NinjaOne. **Correzione del 07/09/2026**: questa riga dichiarava XTS-AES 128 bit; `manage-bde -status` sulla postazione dell'IT Manager misura XTS-AES 256 con cifratura del solo spazio utilizzato e protezioni TPM piu' password numerica. Il cifrario effettivo sull'intera flotta non e' verificato, quindi il valore non va riportato come uniforme. Va inoltre tenuta distinta la presenza della chiave in deposito dalla cifratura del volume: il campo personalizzato `bitlockerKey` su NinjaOne prova l'escrow, non lo stato di cifratura del disco; manca pero' Microsoft Intune (MDM) perche' la licenza M365 e' Business Standard, non Premium — nessuna policy di conformita' dispositivo centralizzata oltre l'AV Bitdefender (GAP-TBC #113, SEC-014) |
| A.8.13 Information backup | Documentato | Inventario NAS fleet consolidato in `vendor-management.md` §QNAP – NAS (RAID, capacita', backup per dispositivo); NAS HERO replicato offsite su Azure Blob Storage. **Correzione dell'08/09/2026, e la riga passa da Documentato a Parziale**: l'inventario dei lavori di backup letto sull'interfaccia dell'apparato ne conta tre, e la replica verso lo spazio a blocchi **non esiste** fra quelli attivi. Era un'affermazione ereditata da un documento di architettura dichiarato obsoleto dall'IT Manager. Lo stato reale, misurato: una sola copia **fuori sede**, verso lo spazio cloud del fornitore del NAS nella regione italiana, e due copie **locali** verso il NAS di appoggio nello stesso edificio. Il perimetro della copia fuori sede e' piu' stretto di quello locale, cioe' due anni contro tre, quindi un anno di documenti esiste in due copie che stanno entrambe in sede. E il [TBC] sulla verifica dei ripristini non e' piu' soltanto non documentato: la funzione di verifica dell'integrita' esiste nel prodotto e su tutti e tre i lavori riporta **mai eseguita**, quindi la restorabilita' delle copie non e' incerta per mancanza di procedura, e' ignota per mancanza di misura. Dettaglio in #192. **Aggiornamento del 16/09/2026, e sposta il problema dal backup dei documenti a quello delle macchine virtuali.** La catena di backup del nodo Proxmox e' stata descritta per la prima volta (otto lavori quotidiani seriali, ritenzione di tre giornaliere piu' una settimanale e una mensile, notifica solo sul fallimento) perche' due suoi lavori sono **falliti** nella notte fra il 15 e il 16/09 per attesa scaduta sul lock. La causa non sta nel backup ma nello storage che legge: la cache del controller RAID del nodo risulta disabilitata, con ogni probabilita' per il guasto del modulo che la protegge, e il pavimento di lettura cronico che ne discende e' circa un decimo di quello atteso (#200). Due conseguenze per questo controllo, e sono di natura diversa. La prima e' che il rimedio applicato, cioe' alzare a ventiquattro ore l'attesa sul lock, **riporta i lavori a completare senza renderli piu' veloci**, e insieme spegne l'unica notifica esistente perche' quella scatta solo sul fallimento: il controllo torna verde mentre la condizione peggiora, che e' il caso peggiore per un controllo di monitoraggio (A.8.16). La seconda e' che il tempo di ripristino dichiarato dalla scheda di continuita' **non e' mai stato misurato** su questo stato dello storage, e va misurato ripristinando una macchina piccola invece che stimato. Riguarda anche A.8.14 e A.5.30. |
...
| A.8.24 Use of cryptography | Non verificato | Nessuna policy crittografica documentata a livello aziendale; a livello di rete, il wildcard SSL su Plesk/Fastnet ha un limite tecnico noto (vedi `scenia-project.md` §Architettura domini) e la VM interna "intrasite" (VM206) usa un certificato auto-firmato, coerente con l'uso puramente interno. **Aggiornamento 27/07/2026**: il pilota sulla VM208 introduce una terza catena di fiducia interna, una CA generata dal reverse proxy la cui root va importata a mano su ogni client, senza inventario delle macchine coinvolte ne' procedura di revoca o rinnovo (SEC-016, GAP-TBC #119). Tre soluzioni diverse per lo stesso problema — certificato pubblico, auto-firmato, CA interna — Tre soluzioni diverse per lo stesso problema — certificato pubblico, auto-firmato, CA interna — sono il segnale che la policy crittografica va scritta prima che i servizi interni si moltiplichino. **Aggiornamento dell'08/09/2026, due fatti in direzioni opposte.** Il primo peggiora il quadro: la sola copia di backup **fuori sede** riporta la cifratura lato client **disabilitata**, quindi i documenti dell'azienda e dei suoi clienti risiedono su un servizio di terze parti in forma leggibile da chi amministra quel servizio e da chiunque ottenga l'accesso all'account (#192). Il secondo lo migliora, ed e' la prima parte di policy crittografica che questo progetto scrive: **ADR-024** fissa lo schema dei nomi interni sotto un sottodominio del dominio aziendale, e **ADR-025** stabilisce che la chiave dell'autorita' di certificazione aziendale viva **fuori linea e protetta da passphrase**, su nessuna macchina della rete. La decisione nasce dalla misura di #179, cioe' un'autorita' interna esistente la cui chiave privata risiede senza passphrase su una macchina senza filtri. Le due cose vanno decise insieme e non in sequenza: attivare la cifratura del backup introduce una chiave la cui perdita rende il backup inutile, quindi la sua custodia ricade sotto la stessa policy di ADR-025 e non va improvvisata dopo |
