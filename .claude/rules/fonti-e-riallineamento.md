...
La terza discende dalle prime due: se l'obiettivo dichiarato e' spostare il firewall da classe E a classe B, cioe' renderlo **leggibile** senza screenshot, il candidato e' il *Cloud Monitoring Mode*, che non rischia nulla. Resta da verificare, e non va dato per scontato, **che cosa esponga davvero la OpenAPI per un firewall in quella modalita'**: la documentazione non lo chiarisce, e un apparato monitorato ma gestito localmente potrebbe comparire nell'inventario senza esporre gli endpoint di configurazione che a questo progetto servirebbero. La prova costa poco una volta assegnato il firewall a un sito, e va fatta prima di annunciare che la fonte ha cambiato classe.
