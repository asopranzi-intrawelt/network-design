...
Legge `data/scadenze.json`, tracciato, che porta i dati: le scadenze con il loro preavviso, le cadenze delle fonti, e le asserzioni che nessun programma puo' verificare con la loro validita' dichiarata. Gli invarianti strutturali stanno invece nello script, perche' sono codice e non configurazione: gli script presenti in `scripts/` e non citati in `STACK.md`, gli ADR richiamati e non definiti, i difetti richiamati oltre il massimo del registro, e le schede il cui `last-verified` e' dietro a HEAD. Gira a ogni avvio di sessione tramite hook, e' **rumoroso e non blocca** per decisione dell'IT Manager, e distingue nel codice di uscita il giallo dal rosso: il giallo e' un'affermazione che sta invecchiando e non marca la sessione come fallita, perche' un controllo che fallisce sempre smette di essere letto; il rosso e' qualcosa di gia' rotto.
